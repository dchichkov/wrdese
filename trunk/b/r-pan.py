...
# -5 : self-revert
    ...
    for e in revisions:
        ...
        karma = user_reputations[e.username]
        if(karma > 1000): f += '9'
        elif(karma > 100): f += '8'
        elif(karma > 20): f += '7'
        elif(karma > 0): f += '6'
...
def analyse_maxent(revisions, user_reputations):
    ...
    train = compute_letter_trainset(revisions, user_reputations)
...
def analyse_decisiontree(revisions, user_reputations):
    ...
    for e in revisions:
        ...
        if(e.comment):
            if(e.comment[:5] == '[[WP:'):
                if e.comment.startswith(u'[[WP:UNDO|Undid'):  score += 0            # TODO
                elif e.comment.startswith(u'[[WP:A'):
                ...
                comment = comment_score(e.comment)
                pass
        else:
            score -= 0 # TODO
            no_comment = True
        ...
        # unexplained removal
        if(no_comment and e.ipedit and blanking): score -= 1; show = True
#                
#
...
#
        karma = user_reputations[e.username]
        if(karma > 1000): score += 1000
        elif(karma > 100): score += 50
        #elif(karma > 20): score += 20
        #elif(karma > 0): score += 1
        elif(karma < -1000): score += 1000
        #elif(karma < -100): pass
        #elif(karma < -20): score -= 20
        #elif(karma < 0): score -= 1
        #else: pass
        ...
        #if(score == 'unknown' and known == 'good'): continue
...
def main():
    ...
    if(_analyze_arg):        
        #user_reputations = check_reputations(revisions, user_reputations)        
